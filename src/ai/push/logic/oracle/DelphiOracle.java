...
public class DelphiOracle extends Oracle {
	...
	private final int[] DISTANCES_RANKS = new int[] {
			1, 16, 36, 56, 80, 110, 144, 176, 216, 266, 326, 396, 466
	};
	...
	public int getProphecy(Transition transition, PLAYER player) {
		...
		if (player == PLAYER.PLAYER1 && (transition.mainMove.angle == 7 || transition.mainMove.angle == 0 || transition.mainMove.angle == 1)) {
			prophecy /= 10;
			prophecy *= 7;
		}
		else if (player == PLAYER.PLAYER2 && (transition.mainMove.angle == 5 || transition.mainMove.angle == 4 || transition.mainMove.angle == 3)) {
			prophecy /= 10;
			prophecy *= 7;	
		}
