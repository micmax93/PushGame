...
public class ExponentialOracle extends Oracle {

	private static final double BASE = 2.7f; // 1.2
	private static final double SCALE = 1.0f;

	public ExponentialOracle(int player1, int player2) {
			...
			for (int c = 0; c < size; ++c) {
				if (transition.out.tab[r][c] == plr) {
					if (plr == player1) { // player at the top of the board
						prophecy += SCALE * Math.pow(BASE, r);
					} else {
						prophecy += SCALE * Math.pow(BASE,  size - 1 - r);
					}
				}
			}
		}
